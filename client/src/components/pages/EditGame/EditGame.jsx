...
const ListPoint = styled.div`
  border-radius: 3px;
  background-color: ${({ theme }) => theme.theme.background.list};
...
const getAllChapters = (id) =>
  ChapterService.getChaptersFromGame(id).then((allChapters) => allChapters)
...
function EditGame({ loggedInUser, match, history }) {
  ...
  useEffect(() => {
    GameService.getOneGame(match.params.gameId).then((game) => {
      !isOwner(game.creator, loggedInUser._id) &&
        history.replace(`/read/${match.params.gameId}`)
